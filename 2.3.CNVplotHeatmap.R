...
#stateData <- subSegCNVs[currAss,]; sampleSubNames <- noNorNames   
...
setNames <- setNames[c(1:16,20:23)]
...
for(currSeg in 1:length(setNames)){
  ...
  for(currEnt in 1:nrow(cnvTab)){
    #is this entry a gene location or arm?
    if(cnvTab[currEnt, "gene"] != "N"){
      geneFlag <- 1
    }else{
      geneFlag <- 0
    }
    ...
    #if gene loci get coordinates directly, if arm get by interval
    if(geneFlag == 1){
      lociCoord <- cnvTab[currEnt, 1]
      testCoord <- subChromPos[subChromPos[["desc"]]==lociCoord, c("start", "end")]
    }else{
      testCoord <- c(subChromPos[1, "start"], subChromPos[nrow(subChromPos), "end"])
    }
    
    #subset CNVs for current chromosome
    subSegCNVs <- filteredSegCNVs[filteredSegCNVs[["chr"]]==currChr, ]
    
    if(nrow(subSegCNVs) > 0){
      #assign arm locations and state to each CNVs
      for(currAss in 1:nrow(subSegCNVs)){
        existsFlag <- 0
        currLociSize <- (testCoord[2] - testCoord[1]) / 1000000
        propCNVtotal <- subSegCNVs[currAss, "nloci"] / currLociSize
        propCNV1 <- ((subSegCNVs[currAss, "last.locus"]  - testCoord[1]) / 1000000) / currLociSize
        propCNV2 <- ((subSegCNVs[currAss, "first.locus"]  - testCoord[2]) / 1000000) / currLociSize
        if(subSegCNVs[currAss, "first.locus"] >= testCoord[1] & subSegCNVs[currAss, "last.locus"] <= testCoord[2] & propCNVtotal >= 0.5){
          #CNV is a focal change on this arm/loci and is greater than 50% of the region
          existsFlag <- 1
        }else if(subSegCNVs[currAss, "first.locus"] <= testCoord[1] & subSegCNVs[currAss, "last.locus"] >= testCoord[2]){
          #CNV spans over the arm / loci
          existsFlag <- 2
        }else if((subSegCNVs[currAss, "first.locus"] <= testCoord[2] & subSegCNVs[currAss, "first.locus"] >= testCoord[1] & propCNV2 >= 0.5) | (subSegCNVs[currAss, "last.locus"] >= testCoord[1] & subSegCNVs[currAss, "last.locus"] <= testCoord[2] & propCNV1 >= 0.5)){
          #CNV spans part of the arm / loci
          existsFlag <- 3
        }else{
          #CNV not on this arm 
          next()
        ...
        #determine whether CNV is gain or loss  
        currState <- getCNVstate(subSegCNVs[currAss,], noNorNames)
...
pdf(file=outputLoc, width=4, height=4)
