...
assessModels <- function(parseModelSub, CNVlistSub, setNamesTemp, dirOutSub){
  ...
  if(nrow(parseModelSub) != 1){
    ...
    write.table(parseModelSub, file=paste(holdingDir, currSet,".bestFittingModels.txt", sep=""), sep="\t", quote = FALSE, col.names = TRUE)
  ...
  write.table(bestModel, file=paste(holdingDir, currSet, "bestModel.txt"), sep="\t", row.names=TRUE, col.names = TRUE, quote = FALSE)
...
modelCNVs <- function(assessSeg, setNamesSub, noSamplesSub, holdingSub){
  ...
  if("overlapping" %in% CNVlist[["phyloLoc"]] & nrow(CNVlist) < 8 & nrow(CNVlist) > 1){
    ...
    write.table(modelProp, file=paste(holdingDir, currSet, ".scoreTab.txt"), sep="\t", quote = FALSE, row.names=TRUE, col.names = TRUE)
    ...
    for(currModName in modNamesList){
      ...
      write.table(currModelProp, file=paste(holdingDir, currSet, ".modResTab.txt"), sep="\t", quote = FALSE, row.names=FALSE, col.names = TRUE)
    }
...
sampleList <- read.table(file="/Users/cross01/Desktop/simSampleList.txt", header=FALSE, stringsAsFactors=FALSE, sep="\t")

sampleNames <- unique(sampleList[[1]])

holdingDir <- "/Users/cross01/PhD/CRCproject/23.MP-CNA/"
fileApp <- ".txt"
plotDir <- "/Users/cross01/PhD/CRCproject/23.MP-CNA/"

eventsTab <- read.table(sep="\t", file="/Users/cross01/Desktop/eventsList.txt", stringsAsFactors = FALSE)


chromoLength <- read.csv(file="~/PhD/ReferenceGenome/chromosomeStartEnd.csv", header=TRUE, colClasses=c(rep("numeric")))

chromoLength <- data.frame(matrix(NA, nrow = 10, ncol = 4))
names(chromoLength) <- c("chromosome", "start", "end", "size")
chromoLength[1] <- c(1:10)
chromoLength[2] <- 1
chromoLength[3] <- 60
chromoLength[4] <- 60
...
for(currSamp in 1:100){
  ...
  segName <- paste(holdingDir, "phy", currSet, fileApp, sep="")
  ...
  for(currChrom in 1:nrow(chrAbTable)){
    ...
    tabDir <- paste(holdingDir, currSet, ".chr", currChrom, sep="")
  ...
  #write table
  mergedOut <- paste(holdingDir, subSample[1,1], ".phyloCNVs.csv" ,sep="")
  write.table(mergedCNVTable, file=mergedOut, sep=",", quote = FALSE, col.names = TRUE, row.names=FALSE)
  
  #write loads table
  loadOut <- paste(holdingDir, subSample[1,1], ".CNVsLoads.csv" ,sep="")
  ...
  #compare merged events to known evolution
  remCols <- which(names(mergedCNVTable) %in% c("ID", "phyloLoc", "nloci", "model"))
  mergedCNVTable <- mergedCNVTable[,-c(remCols)]
  mergedCNVTable[mergedCNVTable[[1]]==1, 1]<- "A"
  mergedCNVTable[mergedCNVTable[[1]]==2, 1]<-"B"
  mergedCNVTable[mergedCNVTable[[1]]==3, 1]<-"C"
  mergedCNVTable[mergedCNVTable[[1]]==4, 1]<-"D"
  mergedCNVTable[mergedCNVTable[[1]]==5, 1]<-"E" 
  mergedCNVTable[mergedCNVTable[[1]]==6, 1]<-"F"
  mergedCNVTable[mergedCNVTable[[1]]==7, 1]<-"G"
  mergedCNVTable[mergedCNVTable[[1]]==8, 1]<-"H" 
  mergedCNVTable[mergedCNVTable[[1]]==9, 1]<-"I" 
  mergedCNVTable[mergedCNVTable[[1]]==10, 1] <- "J" 

  eventTemp <- eventsTab[currSamp, ]
  eventTemp <- eventTemp[!is.na(eventTemp)]
  clonalInfo <- strsplit(as.character(eventTemp), split ="-")
  tempStor <- data.frame(matrix(unlist(clonalInfo), nrow=length(clonalInfo), ncol=5, byrow = TRUE))
  tempStor[6:ncol(mergedCNVTable)] <- 0
  names(tempStor) <- names(mergedCNVTable)
  tempStor[6] <- tempStor[5] 
  tempStor[5] <- unlist(strsplit(as.character(tempStor[[4]]), split = ":"))[seq(2,nrow(tempStor)*2,2)]
  tempStor[4] <- unlist(strsplit(as.character(tempStor[[4]]), split = ":"))[seq(1,nrow(tempStor)*2,2)]
  tempStor[6] <- as.character(tempStor[[6]])
  
  for(currRow in 1:nrow(tempStor)){
    tempStoCh <- tempStor[currRow, 6]
    tempStor[currRow, 6] <- 0
    tempStoCh <- strsplit(as.character(tempStoCh), split = ",")[[1]]
    tempStor[currRow, tempStoCh] <- 1
  }
  tempStor <- as.matrix(tempStor)
  tempStor <- tempStor[order(tempStor[,1], tempStor[,2]), ]
  mergedCNVTable <-  as.matrix(mergedCNVTable)
  
  recordEqual <- c()
  for(currA in 1:nrow(tempStor)){
    testString <- mergedCNVTable[mergedCNVTable[,"chr"]==as.character(tempStor[currA, "chr"]) & mergedCNVTable[,"first.locus"]==as.numeric(tempStor[currA, "first.locus"]), ]
    testString <- paste(testString, collapse = ":")
    
    if(paste(tempStor[currA,], collapse = ":") == testString){
      recordEqual[currA] <- 1
    }else{
      recordEqual[currA] <- 0
    }
  ...
  if(sum(recordEqual) == nrow(mergedCNVTable)){
    idenCounter[currSamp] <- 1
  }else{
    idenCounter[currSamp] <- 0
  }
  iden95Coutner[currSamp] <- sum(recordEqual) / nrow(tempStor)
  pdf(file="/Users/cross01/Desktop/clonalMP-CNA.pdf", width = 5, height = 5)
  barplot(iden95Coutner)
  dev.off()
}
