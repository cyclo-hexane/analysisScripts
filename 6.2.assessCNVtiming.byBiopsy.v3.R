...
for(currCan in 2:length(sampleNames)){
  ...
  colList <- c("red", "goldenrod", "orchid")
  
  outputLoc <- paste(currentSample[1,6], CNVtimingDir, currentSample[1,1], "/", currentSample[1,1], ".CNVtiming.pdf", sep="")
  ...
  for(currChr in 1:23){    
    ...
    noCNVids <- unique(currSub[["CNV_ID"]])
    
    #annotation table
    annoTab <- data.frame(matrix(NA, nrow=length(noCNVids), ncol=4))
    ...
    for(currPoint in length(noCNVids):1){
      
      #subset parameter list
      subParam <- unique(paramData[paramData[["CNV.ID"]]==noCNVids[currPoint], c("start", "end")])
      
      #subset chromosome list for this CNV ID and get mean values
      timingInfo <- currSub[currSub[["CNV_ID"]]==noCNVids[currPoint], ]
      ...
      lines(x=c(timingInfo[1, "t1_est_single"], upperInter), y=rep(tempPlotCounter, 2), col=tempCol, lwd=0.5, lty=1)
      
      lowerInter <- timingInfo[1, "t1_est_single"] - (sqrt(timingInfo[1, "t1_MSE_single"]) * 2)
      lines(x=c(timingInfo[1, "t1_est_single"], lowerInter), y=rep(tempPlotCounter, 2), col=tempCol, lwd=0.5, lty=1)
      
      #main timing point
      points(x=timingInfo[1, "t1_est_single"], y=tempPlotCounter, pch=bioRep, lty = 0, col=tempCol)
  ...
  #add bifurcation time estimates
  biPoint <- bifurcationTable[currentSample[1,1], 1]
  biMSE <- bifurcationTable[currentSample[1,1], 2]
  upperBifur <- biPoint + (1.96 * (sqrt(biMSE) * 1.96)) 
  lowerBifur <- biPoint - (1.96 * (sqrt(biMSE) * 1.96)) 
  
  #add bifurcation points and CI
  plotCounter <- plotCounter -2
  lines(x=rep(biPoint, 2), y=c(0, 23), col="purple", lwd=2, lty=1)
  lines(x=rep(upperBifur, 2), y=c(0, 23), col="violet", lwd=1, lty=1)
  lines(x=rep(lowerBifur, 2), y=c(0, 23), col="violet", lwd=1, lty=1)
