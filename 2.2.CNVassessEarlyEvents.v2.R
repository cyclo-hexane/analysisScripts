...
for(currSam in 1:length(setNames)){
  ...
  if(subSample[1,1] == "Ha.22" | subSample[1,1] == "Ha.12" | subSample[1,1] == "Ha.6" | subSample[1,1] == "Ha.4"){
    subSample[2] <- paste("X", subSample[[2]], sep="")
  }
  ...
  dataIn <- dataIn[dataIn[["phyloLoc"]]=="T" | dataIn[["phyloLoc"]]=="BiT" | dataIn[["phyloLoc"]]=="B" | dataIn[["phyloLoc"]]=="BiB" | dataIn[["phyloLoc"]]=="L", ]
    ...
    for(currType in c("Tet", "Tri", "LOH", "Loss")){
      ...
      if(dataIn[assessRow, currType] != 0){
        if(dataIn[assessRow, "nloci"] > sizeCutoff){
          ...
          }else if(dataIn[assessRow, "phyloLoc"] == "B" | dataIn[assessRow, "phyloLoc"] == "BiB" | dataIn[assessRow, "phyloLoc"] == "L"){
            #CNV is large branch
...
stat1 <- wilcox.test(plotLargeGains[["ClonalTet"]], plotLargeGains[["BTet"]])$p.value
stat2 <- wilcox.test(plotLargeGains[["ClonalTet"]], plotLargeGains[["LTet"]])$p.value
stat3 <- wilcox.test(plotLargeGains[["ClonalTri"]], plotLargeGains[["BTri"]])$p.value
stat4 <- wilcox.test(plotLargeGains[["ClonalTri"]], plotLargeGains[["LTri"]])$p.value
stat5 <- wilcox.test(plotLargeGains[["ClonalLOH"]], plotLargeGains[["BLOH"]])$p.value
stat6 <- wilcox.test(plotLargeGains[["ClonalLOH"]], plotLargeGains[["LLOH"]])$p.value
stat7 <- wilcox.test(plotLargeGains[["ClonalLoss"]], plotLargeGains[["BLoss"]])$p.value
stat8 <- wilcox.test(plotLargeGains[["ClonalLoss"]], plotLargeGains[["LLoss"]])$p.value

#stats to compare small CNVs
stat1s <- wilcox.test(plotSmallGains[["ClonalTet"]], plotSmallGains[["BTet"]])$p.value
stat2s <- wilcox.test(plotSmallGains[["ClonalTet"]], plotSmallGains[["LTet"]])$p.value
stat3s <- wilcox.test(plotSmallGains[["ClonalTri"]], plotSmallGains[["BTri"]])$p.value
stat4s <- wilcox.test(plotSmallGains[["ClonalTri"]], plotSmallGains[["LTri"]])$p.value
stat5s <- wilcox.test(plotSmallGains[["ClonalLOH"]], plotSmallGains[["BLOH"]])$p.value
stat6s <- wilcox.test(plotSmallGains[["ClonalLOH"]], plotSmallGains[["LLOH"]])$p.value
stat7s <- wilcox.test(plotSmallGains[["ClonalLoss"]], plotSmallGains[["BLoss"]])$p.value
stat8s <- wilcox.test(plotSmallGains[["ClonalLoss"]], plotSmallGains[["LLoss"]])$p.value


############ 3.plot graphs clonal vs subclonal ###############
plotCols <- c(rep("red4", 3), rep("red", 3), rep("blue", 3), rep("lightsteelblue", 3))
pdf(file=paste(subSample[1,6], CNVout, "CRC.segSizeDist.pdf", sep=""), width=10, height=8)
